...
export const TICKET_EXTRACTION_SYSTEM_PROMPT = [
  ...
  '[Proibições] Não busque informações, não calcule valores ausentes e não invente datas, moeda, status, valores ou casa. Não escolha layout nem bookmaker. Para esporte, só identifique quando houver evidência visual suficiente; caso contrário use null e registre a dúvida.',
  ...
  '[Esporte] Identifique o esporte quando ele estiver indicado ou for inequivocamente reconhecível pela evidência visível do bilhete (rótulo, ícone, competição, mercado ou contexto textual claro). O campo continua editável pelo usuário no Mini App/Web. Não use pesquisa externa; se houver ambiguidade, conflito ou apenas nomes de equipes sem evidência suficiente, use null e registre um warning.',
