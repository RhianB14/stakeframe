...
describe('durable extraction inbox', () => {
  ...
  it('commits image and queue job together; replay does not download again', async () => {
    ...
    const data = input();
    const id = await store.accept(tenantContext, data, download);
    expect(await store.accept(tenantContext, data, download)).toBe(id);
    expect(download).toHaveBeenCalledTimes(1);
    expect((await boss.getJobById(EXTRACTION_QUEUE, id))?.data).toEqual({
      nonce: id,
    ...
    expect(await store.accept(tenantContext, input(), download)).not.toBe(id);
  });
  ...
  it('ignores late completions from an earlier extraction attempt', async () => {
    const store = integrationStore(database, boss);
    const id = await store.accept(tenantContext, input(), async () => image);
    const first = (await store.claim(tenantContext, id))!;
    await database.pool.query(
      "update integration.inbox set state='pending',version=version+1 where id=$1",
      [id],
    );
    const second = (await store.claim(tenantContext, id))!;
    expect(second.attempt).toBe(first.attempt + 1);
    await store.complete(tenantContext, id, first.attempt, { stale: true });
    await store.fail(tenantContext, id, first.attempt, 'AI_CONNECTION_FAILED');
    expect(
      (
    ...
    await store.complete(tenantContext, id, second.attempt, { current: true });
    expect(
  ...
  it('recovers an interrupted call for explicit review without repeating the paid request', async () => {
    const store = integrationStore(database, boss);
    const id = await store.accept(tenantContext, input(), async () => image);
    await store.claim(tenantContext, id);
    await database.pool.query(
      "update integration.inbox set updated_at=now()-interval '4 minutes' where id=$1",
      [id],
    );
    await store.recoverInterrupted(tenantContext);
    expect(await store.claim(tenantContext, id)).toBeNull();
    expect(
      (
