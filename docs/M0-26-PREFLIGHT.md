...
> **STATUS: preflight + mutações B, C e C2 registradas; mutação D executada
> uma única vez na STK-M0-27, com falha de pré-condição de ambiente antes do
> restore — produção ilesa, ver `docs/M0-27-VALIDATION.md`.** Nenhum segredo, valor de
...
| D   | Executar o primeiro restore isolado | **Executada na STK-M0-27 (execução única)**: falha de pré-condição de ambiente antes do restore — `/run/stakeframe-restore` ausente (provisionado apenas pelo `RuntimeDirectory` do unit da mutação E, não instalada); produção ilesa, cleanup íntegro, zero resíduos — ver `docs/M0-27-VALIDATION.md`. Retorno ao Codex para decisão sobre o pré-requisito (janela D da seção 7, gates 1-5, sem merge-e-executa)                                                                                                                                                   |
