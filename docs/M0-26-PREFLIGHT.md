...
| D   | Executar o primeiro restore isolado | **Pendente — a primeira tentativa autorizada não alcançou o objetivo**: a execução única na STK-M0-27 falhou antes da criação dos recursos de restore (`/run/stakeframe-restore` ausente; provisionado apenas pelo `RuntimeDirectory` do unit da mutação E, não instalada); produção ilesa, cleanup íntegro, zero resíduos; nova execução exige correção revisada do runner e nova autorização operacional — ver `docs/M0-27-VALIDATION.md`. Janela D da seção 7, gates 1-5, sem merge-e-executa                                                                    |
