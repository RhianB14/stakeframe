...
export const bet = finance.table(
  ...
  (t) => [
    ...
    check('bet_completion_state', sql`${t.completionState} in ('incomplete','complete')`),
    check(
      'bet_complete_fields',
      sql`${t.completionState}='incomplete' or (${t.bookmakerId} is not null and ${t.stake} is not null and ${t.odds} is not null and ${t.remaining} is not null and ${t.stakeJournalId} is not null and ${t.stake}>0 and ${t.odds}>=1 and ${t.remaining}>=0 and ${t.remaining}<=${t.stake})`,
    ),
